(() => {
  ...
  const initPlanDetailPanel = (opts = {}) => {
    ...
    const ensureFunding2Panel = () => {
      ...
      clone.querySelectorAll('[data-plan-detail-topup-trigger]').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
      ...
      const syncFromOriginal = () => {
        ...
        if (isPeriodView) {
          ...
          const maxPeriodByBalance = perBuy > 0 ? Math.floor(availBalance / perBuy) : 0;
          const staticMaxPeriod = getFunding2StaticMaxPeriod(reserveCur, perBuy, availBalance);
    ...
    const initPlanBufferPanel = () => {
      ...
      const periodRangeEl = bufferPanel.querySelector('[data-plan-buffer-period-range]');
      ...
      const getMaxAllowedAmount = () => {
        ...
        const hardCap = Math.max(0, Math.min(balance, maxByBuys, MAX_RESERVE_INPUT));
      ...
      const render = () => {
        ...
        const balance = BALANCES[cur] ?? BALANCES.TWD;
        const staticAmountCap = cur === 'USDT'
          ? 100000
          : cur === 'TWD'
            ? 3000000
        ...
        if (periodRangeEl) {
          const maxPeriodCount = perBuy > 0 ? Math.floor(staticAmountCap / perBuy) : 0;
    ...
    const initThemeGuideSheet = () => {
      ...
      const resolveCategory = (key) => {
        ...
        if (visibleCategories.some((c) => c.key === normalized)) return normalized;
      ...
      const renderCards = () => {
        cardsEl.innerHTML = visibleCategories.map((cat) => `
      ...
      const setActiveCategory = (key, opts = {}) => {
        ...
        if (opts.scrollCard !== false) {
          const idx = visibleCategories.findIndex((cat) => cat.key === activeCategory);
          const cardEls = Array.from(cardsEl.querySelectorAll('[data-theme-guide-card]'));
          if (idx >= 0 && cardEls[idx]) {
            const prevIdx = visibleCategories.findIndex((cat) => cat.key === prevActiveCategory);
